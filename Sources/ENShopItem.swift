...
struct ENShopItem: JSONDeserializable {
    var key: String
    var price: String
    var count: Int
    var position: Int = 0
    ...
    static func deserialize(fromJSON json: JSON) -> ENShopItem {
        let key = json[kKey].string!
        let price = json["price"].string!
        let count = json["count"].int ?? 1
        var ret = ENShopItem(key: key, price: price, count: count, displayName: json["displayname"].string!)
